...
const soundLibrary = await evaluate(`(() => {
  let library = document.querySelector('.sound-library');
  const initiallyClosed = !library.open;
  library.querySelector('.sound-library-help').click();
  const helpTitle = document.querySelector('#help-title').textContent;
  const helpDoesNotOpenLibrary = !library.open;
  document.dispatchEvent(new KeyboardEvent('keydown', { key: 'Escape', bubbles: true }));
  library.open = true;
  const groups = library.querySelectorAll('.sound-library-group').length;
  const buttons = library.querySelectorAll('.sound-preset-button').length;
  const soundscape = document.querySelector('textarea[data-global="soundscape"]');
  const soundscapeBefore = soundscape.value;
  soundscape.focus();
  soundscape.setSelectionRange(soundscape.value.length, soundscape.value.length);
  [...library.querySelectorAll('.sound-preset-button')].find((item) => item.textContent.includes('稳定雨声')).click();
  const soundscapeInserted = document.querySelector('textarea[data-global="soundscape"]').value.includes('Steady rain falls in the background.');
  const soundscapeOutput = document.querySelector('#prompt-output').textContent.includes('Steady rain falls in the background.');
  document.querySelector('#undo-action').click();
  const soundscapeUndone = document.querySelector('textarea[data-global="soundscape"]').value === soundscapeBefore;
  library = document.querySelector('.sound-library');
  library.open = true;
  [...library.querySelectorAll('.sound-preset-button')].find((item) => item.textContent.includes('稀疏钢琴')).click();
  const musicEnabled = !document.querySelector('[data-global="noMusic"]').checked;
  const musicField = document.querySelector('textarea[data-global="music"]');
  const musicInserted = musicField?.value.includes('Sparse piano notes at a slow tempo');
  const musicOutput = document.querySelector('#prompt-output').textContent.includes('Sparse piano notes at a slow tempo');
  document.querySelector('#undo-action').click();
  const musicUndone = document.querySelector('[data-global="noMusic"]').checked && !document.querySelector('textarea[data-global="music"]');
  return { initiallyClosed, helpTitle, helpDoesNotOpenLibrary, groups, buttons, soundscapeInserted, soundscapeOutput, soundscapeUndone, musicEnabled, musicInserted, musicOutput, musicUndone };
})()`);
check("sound library inserts soundscape phrases and automatically enables non-diegetic music presets", soundLibrary.initiallyClosed && soundLibrary.helpTitle === '声音快速库' && soundLibrary.helpDoesNotOpenLibrary && soundLibrary.groups === 4 && soundLibrary.buttons === 32 && soundLibrary.soundscapeInserted && soundLibrary.soundscapeOutput && soundLibrary.soundscapeUndone && soundLibrary.musicEnabled && soundLibrary.musicInserted && soundLibrary.musicOutput && soundLibrary.musicUndone, JSON.stringify(soundLibrary));
const starterDefaults = await evaluate(`(() => {
  const promptBefore = document.querySelector('#prompt-output').textContent;
  document.querySelector('[data-action="add-summary"]').click();
  const addedSummary = [...document.querySelectorAll('textarea[data-entity="summaries"][data-key="content"]')].at(-1)?.value;
  document.querySelector('#undo-action').click();
  document.querySelector('[data-action="add-subject"]').click();
  const subjectCard = [...document.querySelectorAll('textarea[data-entity="subjects"][data-key="definition"]')].at(-1)?.closest('.item-card');
  const addedSubjectDefinition = subjectCard?.querySelector('textarea[data-key="definition"]')?.value;
  const addedSubjectRetention = subjectCard?.querySelector('textarea[data-key="retention"]')?.value;
  document.querySelector('#undo-action').click();
  document.querySelector('[data-action="add-shot"]').click();
  const addedShot = [...document.querySelectorAll('textarea[data-entity="shots"][data-key="content"]')].at(-1)?.value;
  document.querySelector('#undo-action').click();
  window.confirm = () => true;
  document.querySelector('#reset-mode').click();
  const resetSummary = document.querySelector('textarea[data-entity="summaries"][data-key="content"]')?.value;
  const resetShot = document.querySelector('textarea[data-entity="shots"][data-key="content"]')?.value;
  const resetSoundscape = document.querySelector('textarea[data-global="soundscape"]')?.value;
  const resetStyle = [...document.querySelectorAll('[data-style-preset]:checked')].map((item) => item.dataset.stylePreset);
  const starterWarning = document.querySelector('#validation').textContent.includes('仍是默认起句');
  document.querySelector('#undo-action').click();
  const restored = document.querySelector('#prompt-output').textContent === promptBefore;
  return { addedSummary, addedSubjectDefinition, addedSubjectRetention, addedShot, resetSummary, resetShot, resetSoundscape, resetStyle, starterWarning, restored };
})()`);
check("new sections and reset modes begin with editable standard English starters", starterDefaults.addedSummary === 'The target video shows ' && starterDefaults.addedSubjectDefinition === 'is the referenced visible subject.' && starterDefaults.addedSubjectRetention.startsWith('the referenced identity') && starterDefaults.addedShot === 'the camera cuts to ' && starterDefaults.resetSummary === 'The target video shows ' && starterDefaults.resetShot === 'A close shot frames ' && starterDefaults.resetSoundscape.startsWith('Natural environmental room tone') && starterDefaults.resetStyle.join('|') === 'live-action|cinematic' && starterDefaults.starterWarning && starterDefaults.restored, JSON.stringify(starterDefaults));
